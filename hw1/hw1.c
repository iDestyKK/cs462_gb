...
hw1_params_t hw1_params = {0,};

#define BUFSIZE 1024
#define EPSILON 0.000005

int read_input_file( hw1_params_t* params )
{
    FILE *infile;
    ...
    if( HW1_FLAG_GENERATE_HEAT_IMAGE & params->flags ) {
...
void print_params(hw1_params_t* param)
{
    ...
    if( HW1_FLAG_VERBOSE & param->flags ) {
...
int main(int argc, char* argv[] )
{
    ...
    while( -1 != (c = getopt_long(argc, argv, "vti:", long_opts, &opt_idx)) ) {
        switch (c) {
        case 'v':
            if( HW1_FLAG_VERBOSE & hw1_params.flags )
                hw1_params.flags |= HW1_FLAG_VERY_VERBOSE;
            else
                hw1_params.flags |= HW1_FLAG_VERBOSE;
            break;
        case 't':
            hw1_params.flags |= HW1_FLAG_TIME_ALG;
            break;
        case 'i':
            hw1_params.flags |= HW1_FLAG_GENERATE_HEAT_IMAGE;
            hw1_params.vis_output = strdup(optarg);
            break;
        case '?':
    ...
    hw1_params.input = strdup(argv[optind]);
    read_input_file(&hw1_params);
    print_params(&hw1_params);

    relaxation_params_t* rp = relaxation_init(&hw1_params);
    if( NULL == rp ) {
        fprintf(stderr, "Error initializing the relaxation. Bail out!\n");
        return -1;
    }
    if( HW1_FLAG_GENERATE_HEAT_IMAGE & hw1_params.flags ) {
        relaxation_coarsen(rp, hw1_params.vis_data, hw1_params.vis_res, hw1_params.vis_res);
        dump_gray_image(hw1_params.vis_output, 0,
                        hw1_params.vis_data, hw1_params.vis_res, hw1_params.vis_res);
    }
    ...
    while(1) {
         ...
         * the relaxation time.
         */
        if( 0 == (iter % hw1_params.vis_step) ) {
            double s = wtime();
            if( HW1_FLAG_GENERATE_HEAT_IMAGE & hw1_params.flags ) {
                relaxation_coarsen(rp, hw1_params.vis_data, hw1_params.vis_res, hw1_params.vis_res);
                /*print_matrix(hw1_params.vis_data, hw1_params.vis_res, hw1_params.vis_res);*/
                dump_gray_image(hw1_params.vis_output, iter,
                                hw1_params.vis_data, hw1_params.vis_res, hw1_params.vis_res);
            }
            if( HW1_FLAG_VERY_VERBOSE & hw1_params.flags ) {
        ...
        if( (HW1_FLAG_VERBOSE & hw1_params.flags) && (0 == (iter % hw1_params.vis_step)) ) {
        ...
        if( (hw1_params.max_iterations > 0) && (iter >= hw1_params.max_iterations) ) {
            break;
        }
        iter++;
    }
    elapsed = wtime() - start - ignore;
    if( HW1_FLAG_TIME_ALG & hw1_params.flags ) {
