...
/**
 * This is a oshmem_mp for an empty relaxation that can be used as an
...
struct relaxation_oshmem_mp_hidden_params_s {
...
const struct relaxation_function_class_s _relaxation_oshmem_mp;

void update_vectors(struct relaxation_oshmem_mp_hidden_params_s* rp) {
	//TODO: Implement
...
void sync_vectors(struct relaxation_oshmem_mp_hidden_params_s* rp) {
	//TODO: Implement
	CN_UINT i, j;
	AREA* cr,
	    * rr = &rp->region[rp->id];
...
oshmem_mp_relaxation_init(struct hw_params_s* hw_params)
{
	struct relaxation_oshmem_mp_hidden_params_s* rp;
	uint32_t np = hw_params->resolution + 2;

	rp = (struct relaxation_oshmem_mp_hidden_params_s*)malloc(sizeof(struct relaxation_oshmem_mp_hidden_params_s));
	if( NULL == rp ) {
	...
	rp->super.sizex = np;
	rp->super.sizey = np;
	rp->super.rel_class = &_relaxation_oshmem_mp;
...
static int oshmem_mp_relaxation_fini(relaxation_params_t** prp)
{
	struct relaxation_oshmem_mp_hidden_params_s* rp = (struct relaxation_oshmem_mp_hidden_params_s*)*prp;
	if( NULL != rp->data ) cn_vec_free(rp->data);
...
static int oshmem_mp_relaxation_coarsen(relaxation_params_t* grp, double* dst, uint32_t dstx, uint32_t dsty)
{
	struct relaxation_oshmem_mp_hidden_params_s* rp = (struct relaxation_oshmem_mp_hidden_params_s*)grp;
	return coarsen((double*)rp->data->data, rp->super.sizex, rp->super.sizey,
			dst, dstx, dsty);
}

static int oshmem_mp_relaxation_print(relaxation_params_t* grp, FILE* fp)
{
	struct relaxation_oshmem_mp_hidden_params_s* rp = (struct relaxation_oshmem_mp_hidden_params_s*)grp;
	fprintf( fp, "\n# Iteration %d\n", rp->idx);
...
/**
 * One step of a simple oshmem_mp relaxation.
 */
static double oshmem_mp_relaxation_apply(relaxation_params_t* grp)
{
	...
	#pragma omp parallel for reduction(+:sum) collapse(2) default(none) shared(old,new,rp,cr) private(diff)
...
static double* oshmem_mp_relaxation_get_data(relaxation_params_t* grp)
{
	return (double*)((struct relaxation_oshmem_mp_hidden_params_s*)grp)->data->data;
}


const struct relaxation_function_class_s _relaxation_oshmem_mp =
{
	.type = RELAXATION_JACOBI_OSHMEM_OPENMP,
	._init = oshmem_mp_relaxation_init,
	._fini = oshmem_mp_relaxation_fini,
	._coarsen = oshmem_mp_relaxation_coarsen,
	._print = oshmem_mp_relaxation_print,
	._relaxation = oshmem_mp_relaxation_apply,
	._get_data = oshmem_mp_relaxation_get_data,
};
